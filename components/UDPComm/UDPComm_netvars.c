...
void UDPComm_netvars_append_json(cJSON *root)
{
    ...
    {
        NetVars_append_json(UDPComm_netvars_desc, UDPComm_netvars_count, root);
    }
}
...
void UDPComm_nvs_cfg_save(void)
{
    esp_err_t err;
    // Open
    ESP_LOGI(TAG, "Opening Non-Volatile Storage (NVS) handle... ");
...
void UDPComm_config_parse_json(const char *data)
{
    ...
    if (root != NULL)
    {
        if (data[0] == '[')
        {
            ...
            {
                nvs_cfg_changed = UDPComm_netvars_parse_json_dict(nvi);
            }
        }
        else
        {
            nvs_cfg_changed = UDPComm_netvars_parse_json_dict(root);
        }
        cJSON_Delete(root);
        if (nvs_cfg_changed)
        {
            UDPComm_nvs_cfg_save();
        }
    }
}

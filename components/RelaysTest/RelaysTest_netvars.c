...
void RelaysTest_netvars_append_json(cJSON *root)
{
    ...
    {
        NetVars_append_json(RelaysTest_netvars_desc, RelaysTest_netvars_count, root);
    }
}
...
void RelaysTest_config_parse_json(const char *data)
{
    ...
    if (root != NULL)
    {
        if (data[0] == '[')
        {
            ...
            {
                nvs_cfg_changed = RelaysTest_netvars_parse_json_dict(nvi);
            }
        }
        else
        {
            nvs_cfg_changed = RelaysTest_netvars_parse_json_dict(root);
        }
        cJSON_Delete(root);
        if (nvs_cfg_changed)
        {
